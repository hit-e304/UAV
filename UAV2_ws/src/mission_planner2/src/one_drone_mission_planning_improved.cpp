...
{
    current_pose = *curr_p;
    current_pose.pose.position.x = current_pose.pose.position.x - axis_offset2;
}
...
void other_UAVs_state_cb(const mission_planner2::OtherUAVsStates::ConstPtr& msg)
{
...
{
    ros::init(argc, argv, "formation_uav2");
    ros::NodeHandle nh;

    ros::Subscriber state_sub = nh.subscribe<mavros_msgs::State>("/uav2/mavros/state", 10, state_cb);

    ros::Publisher local_target_pub = nh.advertise<mavros_msgs::PositionTarget>("/uav2/mavros/setpoint_raw/local", 10);
    ros::Subscriber other_UAVs_state_sub = nh.subscribe("/to_uav2", 1, other_UAVs_state_cb);
    
    ros::Subscriber local_position_sub = nh.subscribe("/uav2/mavros/local_position/pose", 1, local_pos_cb);
    ros::Subscriber local_velocity_sub = nh.subscribe("/uav2/mavros/local_position/velocity_body", 1, local_vel_cb);
    ros::Subscriber cam_sub = nh.subscribe("/uav2/contours_topic", 1, cam_subCallback);
    ...
    // UAV3_local_pose = other_UAVs_current_states.other_UAVs_state[1].UAV_pose;

    double distance_UAV1_UAV2 = sqrt(pow(UAV1_local_pose.pose.position.x - current_pose.pose.position.x, 2) + pow(UAV1_local_pose.pose.position.y - current_pose.pose.position.y, 2) + pow(UAV1_local_pose.pose.position.z - current_pose.pose.position.z, 2));
    // double distance_UAV1_UAV3 = sqrt(pow(current_pose.pose.position.x - UAV3_local_pose.pose.position.x, 2) + pow(current_pose.pose.position.y - UAV3_local_pose.pose.position.y, 2) + pow(current_pose.pose.position.z - UAV3_local_pose.pose.position.z, 2));
    double distance_UAV2_UAV3 = sqrt(pow(UAV3_local_pose.pose.position.x - current_pose.pose.position.x, 2) + pow(UAV3_local_pose.pose.position.y - current_pose.pose.position.y, 2) + pow(UAV3_local_pose.pose.position.z - current_pose.pose.position.z, 2));
    double v12_x = 0.0;
    double v12_y = 0.0;
    double v23_x = 0.0;
    double v23_y = 0.0;
    ...
    position_target_local.position.x = -axis_offset2;
    ...
    while(ros::ok()){
        ...
        distance_UAV1_UAV2 = sqrt(pow(UAV1_local_pose.pose.position.x - current_pose.pose.position.x, 2) + pow(UAV1_local_pose.pose.position.y - current_pose.pose.position.y, 2) + pow(UAV1_local_pose.pose.position.z - current_pose.pose.position.z, 2));
        distance_UAV2_UAV3 = sqrt(pow(UAV3_local_pose.pose.position.x - current_pose.pose.position.x, 2) + pow(UAV3_local_pose.pose.position.y - current_pose.pose.position.y, 2) + pow(UAV3_local_pose.pose.position.z - current_pose.pose.position.z, 2));
        if(distance_UAV1_UAV2 < d_threshold)
        {
            v12_x = k_v / (current_pose.pose.position.x - UAV1_local_pose.pose.position.x);
            v12_y = k_v / (current_pose.pose.position.y - UAV1_local_pose.pose.position.y);
        }
        else
        {
            v12_x = 0.0;
        ...
        {
            v23_x = k_v / (current_pose.pose.position.x - UAV3_local_pose.pose.position.x);
            v23_y = k_v / (current_pose.pose.position.y - UAV3_local_pose.pose.position.y);
        }
        else
        {
            v23_x = 0.0;
        ...
        position_target_local.velocity.x = v12_x + v23_x;
        position_target_local.velocity.y = v12_y + v23_y;
        //std::cout << current_pose.pose.position.x << " " << current_pose.pose.position.y << " " << current_pose.pose.position.z << std::endl;
        position_target_local.position.x = position_target_local.position.x + axis_offset2;
        local_target_pub.publish(position_target_local);
