...
//     }
// }
double axis_offset3 = 7;//local坐标系3偏置
...
{
    current_pose = *curr_p;
    current_pose.pose.position.x = current_pose.pose.position.x - axis_offset3;
}
...
void other_UAVs_state_cb(const mission_planner3::OtherUAVsStates::ConstPtr& msg)
{
    UAV1_local_pose = msg->other_UAVs_state[0].UAV_pose;
    UAV2_local_pose = msg->other_UAVs_state[1].UAV_pose;
}
...
{
    ros::init(argc, argv, "formation_uav3");
    ros::NodeHandle nh;

    ros::Subscriber state_sub = nh.subscribe<mavros_msgs::State>("/uav3/mavros/state", 10, state_cb);

    ros::Publisher local_target_pub = nh.advertise<mavros_msgs::PositionTarget>("/uav3/mavros/setpoint_raw/local", 10);
    ros::Subscriber other_UAVs_state_sub = nh.subscribe("/to_uav3", 1, other_UAVs_state_cb);
    
    ros::Subscriber local_position_sub = nh.subscribe("/uav3/mavros/local_position/pose", 1, local_pos_cb);
    ros::Subscriber local_velocity_sub = nh.subscribe("/uav3/mavros/local_position/velocity_body", 1, local_vel_cb);
    ros::Subscriber cam_sub = nh.subscribe("/uav3/contours_topic", 1, cam_subCallback);
    ...
    // UAV1_local_pose = other_UAVs_current_states.other_UAVs_state[0].UAV_pose;
    // UAV2_local_pose = other_UAVs_current_states.other_UAVs_state[1].UAV_pose;

    // double distance_UAV1_UAV2 = sqrt(pow(UAV2_local_pose.pose.position.x - current_pose.pose.position.x, 2) + pow(UAV2_local_pose.pose.position.y - current_pose.pose.position.y, 2) + pow(UAV2_local_pose.pose.position.z - current_pose.pose.position.z, 2));
    double distance_UAV1_UAV3 = sqrt(pow(current_pose.pose.position.x - UAV1_local_pose.pose.position.x, 2) + pow(current_pose.pose.position.y - UAV1_local_pose.pose.position.y, 2) + pow(current_pose.pose.position.z - UAV1_local_pose.pose.position.z, 2));
    double distance_UAV2_UAV3 = sqrt(pow(UAV2_local_pose.pose.position.x - current_pose.pose.position.x, 2) + pow(UAV2_local_pose.pose.position.y - current_pose.pose.position.y, 2) + pow(UAV2_local_pose.pose.position.z - current_pose.pose.position.z, 2));
    // double v12_x = 0.0;
    // double v12_y = 0.0;
    double v23_x = 0.0;
    double v23_y = 0.0;
    ...
    position_target_local.position.x = -axis_offset3;
    ...
    while(ros::ok()){
        ...
        else if(PX4_current_mission_state == TrackingCam)
        {
            ...
            if (camera_data.find_obs_flag)
            {
                position_target_new.position.x = current_pose_new.position.x + 5;
        ...
        distance_UAV1_UAV3 = sqrt(pow(current_pose.pose.position.x - UAV1_local_pose.pose.position.x, 2) + pow(current_pose.pose.position.y - UAV1_local_pose.pose.position.y, 2) + pow(current_pose.pose.position.z - UAV1_local_pose.pose.position.z, 2));
        distance_UAV2_UAV3 = sqrt(pow(UAV2_local_pose.pose.position.x - current_pose.pose.position.x, 2) + pow(UAV2_local_pose.pose.position.y - current_pose.pose.position.y, 2) + pow(UAV2_local_pose.pose.position.z - current_pose.pose.position.z, 2));
    
        if(distance_UAV1_UAV3 < d_threshold)
        {
            v13_x = k_v / (current_pose.pose.position.x - UAV1_local_pose.pose.position.x);
            v13_y = k_v / (current_pose.pose.position.y - UAV1_local_pose.pose.position.y);
        }
        else
        {
            v13_x = 0.0;
        ...
        {
            v23_x = k_v / (current_pose.pose.position.x - UAV2_local_pose.pose.position.x);
            v23_y = k_v / (current_pose.pose.position.y - UAV2_local_pose.pose.position.y);
        }
        else
        {
            v23_x = 0.0;
        ...
        position_target_local.velocity.x = v13_x + v23_x;
        position_target_local.velocity.y = v13_y + v23_y;
        //std::cout << current_pose.pose.position.x << " " << current_pose.pose.position.y << " " << current_pose.pose.position.z << std::endl;
        position_target_local.position.x = position_target_local.position.x + axis_offset3;
        local_target_pub.publish(position_target_local);
