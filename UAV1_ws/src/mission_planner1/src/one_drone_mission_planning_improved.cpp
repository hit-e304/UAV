...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber cam_sub = nh.subscribe("/uav1/contours_topic", 1, cam_subCallback);
    ...
    // UAV2_local_pose = other_UAVs_current_states.other_UAVs_state[0].UAV_pose;
    ...
    // double distance_UAV2_UAV3 = sqrt(pow(UAV3_local_pose.pose.position.x - current_pose2.pose.position.x, 2) + pow(UAV3_local_pose.pose.position.y - current_pose2.pose.position.y, 2) + pow(UAV3_local_pose.pose.position.z - current_pose2.pose.position.z, 2));
    double v12_x = 0.0;
    double v12_y = 0.0;
    // double v23_x = 0.0;
    // double v23_y = 0.0;
    ...
    while(ros::ok()){
        ...
        distance_UAV1_UAV2 = sqrt(pow(UAV2_local_pose.pose.position.x - current_pose.pose.position.x, 2) + pow(UAV2_local_pose.pose.position.y - current_pose.pose.position.y, 2) + pow(UAV2_local_pose.pose.position.z - current_pose.pose.position.z, 2));
        distance_UAV1_UAV3 = sqrt(pow(current_pose.pose.position.x - UAV3_local_pose.pose.position.x, 2) + pow(current_pose.pose.position.y - UAV3_local_pose.pose.position.y, 2) + pow(current_pose.pose.position.z - UAV3_local_pose.pose.position.z, 2));
    
        if(distance_UAV1_UAV2 < d_threshold)
        {
            v12_x = k_v / (current_pose.pose.position.x - UAV2_local_pose.pose.position.x);
            v12_y = k_v / (current_pose.pose.position.y - UAV2_local_pose.pose.position.y);
        }
        else
        {
            v12_x = 0.0;
        ...
        {
            v13_x = k_v / (current_pose.pose.position.x - UAV3_local_pose.pose.position.x);
            v13_y = k_v / (current_pose.pose.position.y - UAV3_local_pose.pose.position.y);
        }
        else
        {
            v13_x = 0.0;
        ...
        position_target_local.velocity.x = v12_x + v13_x;
        position_target_local.velocity.y = v12_y + v13_y;
